...
1. Check if project is already bootstrapped:
   - IF `apps/` exists AND `packages/` exists AND `docker-compose.yml` exists → skip to Step 2
   - ELSE → run `/start`
2. Verify bootstrap succeeded:
   - Project structure exists
   - Docker services can build
...
LOOP:
    ...
    3. Run `/go <feature-name>` to implement the feature
       - /go automatically selects /plan or /feature based on complexity
